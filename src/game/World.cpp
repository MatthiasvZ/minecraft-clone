...
World::World()
    : shader(PT::Shader(PT_SHADER_XYZBUV_M)), lockRenderer(false), stop(false), GLOsMissing(false), mouseHold(0)
{
    ...
    for (int ix {0}; ix < CHUNKRD; ++ix)
        for (int iy {0}; iy < MAXHEIGHT; ++iy)
            for (int iz {0}; iz < CHUNKRD; ++iz)
                chunks.add(new Chunk(ix, iy, iz));

    // The chunk meshes will be generated by the chunk loader
...
void World::drawChunks(float deltaTime, const PT::Window& window, bool mouseLocked)
{
    ...
    for (size_t i {0}; i < chunkMeshes.size(); ++i)
    {
        ...
        if (chunkMeshes[i]->isEmpty())
            continue;
        while (lockRenderer || GLOsMissing)
            if (GLOsMissing)
                createBufferObjects();
